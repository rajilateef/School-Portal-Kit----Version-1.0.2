...
?>
 ...
 <div class="page-title hidden-print">
          <div>
            <h1><i class="fa fa-search"></i> Search Students by Reg number</h1>
          </div>
        </div>
		 <div class="card">
			<div class = 'row'>
								...
								<div class='row'>
									<div class = 'col-xs-12 col-sm-12 col-md-8'>
										<div class = 'input-group'>
											...
											<input type='text' name='regno' placeholder='Enter the student registration number' class='form-control'>
										</div><br/>
									</div>
									<div class = 'col-xs-12 col-sm-12 col-md-4'>
										<button type = 'submit' name = 'search_reg_no_btn' class = 'btn btn-primary btn-md' style='margin-left:20px;'><span class = 'glyphicon glyphicon-user'></span>&nbsp; Search</button><br/> 
									</div>
								</div>
							</form>
                        </div>
			...
			<?php
                //////////////////// POST ACTION TO GET A STUDENT NAMES AND REGISTRATION NUMBER WITH A PARTICULAR NUMBAR //////////////////////
                if(isset($_POST['search_reg_no_btn'])){
                    $regno = inject_checker($connection, $_POST['regno']);

                    if(empty($regno)){
                        $reg_msg = "<h4 class='text-danger'><b>!!! Please type registration number.</b></h4>";
                        echo $reg_msg;
                    }
                    else{
                        $query = " SELECT * FROM `students` WHERE `reg_number` = '{$regno}' ";
                        $run_query = mysqli_query($connection, $query);
                        if($run_query == true){
