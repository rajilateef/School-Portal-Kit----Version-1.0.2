...
<div class="page-title hidden-print">
  <div>
    <h1><i class="fa fa-search"></i> Search Students by Name</h1>
  </div>
</div>
 <div class="card">
    <div class = 'row'>
                        ...
                        <div class='row'>
                            <div class = 'col-xs-12 col-sm-12 col-md-8'>
                                <div class = 'input-group'>
                                    ...
                                    <input type='text' name='name' placeholder='Enter only either first, second or third name' class='form-control'>
                                </div><br/>
                            </div>
                            <div class = 'col-xs-12 col-sm-12 col-md-4'>
                                <button type = 'submit' name='search_by_name_btn' class = 'btn btn-primary btn-md' style='margin-left:20px;'><span class = 'glyphicon glyphicon-user'></span>&nbsp; Search</button><br/> 
                            </div>
                        </div>
                    </form>
                </div>
    ...
    <?php
        ...
        if(isset($_POST['search_by_name_btn'])){
            $name = inject_checker($connection, ucfirst($_POST['name']));
            
            if(empty($name)){
                echo "<p class='text-danger'><b>Name field can not be empty</b></p>";
            }else{
                ...
                if($run_query == true){
                    if(mysqli_num_rows($run_query) > 0){
                        ...
                        $query = " SELECT * FROM `students` WHERE `lastname` = '{$name}' ";
                        $run_query = mysqli_query($connection, $query);
                        if($run_query == true){
                            if(mysqli_num_rows($run_query) > 0){
                                $i = 0;
                            ...
                            $query = " SELECT * FROM `students` WHERE `othername` = '{$name}' ";
                            $run_query = mysqli_query($connection, $query);
                            if($run_query == true){
                                if(mysqli_num_rows($run_query) > 0){
                                    $i = 0;
